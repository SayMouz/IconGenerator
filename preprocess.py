...
data_root = "./ic_data/1000icons_x_5tags/"
...
ic_tags_filepath = data_root + "ic_tags.txt"
class_num = 5
...
num_ic = 4500
ic_size = 32
png_dirpath = data_root + "png/"
jpg_dirpath = data_root + "jpg/"
...
for i in range(num_ic):
    try:
        ...
        img_array = np.where(img_array >= 220, 0, 1)
...
ic_data = data_array.reshape(data_array.shape[0], -1)  # 3D -> 2D
...
np.savetxt(data_root + 'tags_data.csv', tags_data, fmt='%i')

# Save array images
np.savetxt(data_root + "ic_data.csv", ic_data, fmt='%i')

# Save clean data
np.savetxt(data_root + "ic_data_clean.csv", clean_ic_data, fmt='%i')
np.savetxt(data_root + 'tags_data_clean.csv', clean_tags_data, fmt='%i')
